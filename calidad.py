...
Portado de `video-scout-pipeline/calidad.py`. La parte que mide es la misma
—son medidas de ffmpeg, no dependen del pipeline— y lo que cambia es de dónde
sale el registro de videos y a qué apuntan los consejos de arreglo.
...
revisan todos los videos y no una muestra.
...
Todo se mide en UNA sola pasada de ffmpeg: cada pasada es descodificar el
video entero, y cuatro filtros en la misma cadena cuestan lo mismo que uno.
...
# YouTube normaliza a unos -14 LUFS: si subes más bajo, te lo deja como está y
# tu video suena flojo al lado del siguiente; si subes más alto, te lo baja y
# solo has perdido rango dinámico.
LUFS_OBJETIVO = -14.0
...
# Ritmo de la narración, para detectar un TTS que entregó menos texto del que
# se le dio. MEDIDO EN ESTE REPO, no heredado: el guion semilla emocional
# tiene 59 palabras de narración y la corrida 35458025036 lo rindió en 20.0s,
# o sea 2.95 palabras/segundo. Sale más rápido que las 2.6 de
# video-scout-pipeline porque aquí edge-tts corre a velocidad_locucion "+8%".
#
# Es UN solo punto de medida, así que está redondeado a la baja para no
# acusar en falso, y conviene recalibrarlo cuando haya una docena de videos
# reales. Sirve para cazar una narración truncada a la mitad, no para afinar.
...
TIEMPO_MAXIMO = 600      # segundos de margen para descodificar


def _config():
    """La config del repo. Si no se puede cargar se sigue con lo que haya:
    esto es una herramienta de diagnóstico y no debe morirse por eso."""
    try:
        import generar_video_maestro as gvm
        return gvm.cargar_config()
    except Exception:
        return {}


def _perdon_congelado(cfg):
    """Hasta qué segundo se perdona que la imagen esté parada.

    La tarjeta de intro es una imagen fija, así que freezedetect la marca
    siempre: no es un fallo, es el diseño. En los shorts de este repo la
    tarjeta dura 0 segundos (`DURACION_INTRO_CARD_SHORT_SEG`), así que ahí no
    hay nada que perdonar y un congelado al principio sí es un fallo — al
    revés que en el repo hermano, donde la tarjeta existe siempre y el perdón
    estaba fijo en 6 segundos.
    """
    import generar_video_maestro as gvm
    return (gvm.DURACION_INTRO_CARD_SHORT_SEG if formato_video.es_short(cfg)
            else gvm.DURACION_INTRO_CARD_SEG)
...
def _leer(ruta, por_defecto):
    """Un json que puede no existir o estar a medio escribir.

    No se usa `archivos.valido` y ya: eso dice que el archivo pesa algo, no
    que sea json legible. Una corrida cortada a mitad de escritura deja las
    dos cosas, y tratarla como 'sin revisiones' es mejor que reventar.
    """
    try:
...
def revisar(medidas, registro=None, cfg=None):
    """De los números a la lista de lo que está mal.

    `registro` es la entrada de resultado_lote.json, si la hay: de ahí salen
    el texto narrado y el día, que permiten dos comprobaciones más. Sin él se
    revisa igual, solo que con menos cosas.
    """
    hallazgos = []
    reg = registro or {}
    cfg = cfg or {}
    ...
    if palabras >= 40 and dur > 0:
        ...
        if dur < esperado * FRACCION_MINIMA:
            ...
            hallazgos.append(_hallazgo(
                ...
                "regenerar_video en el workflow, o "
                "python pipeline.py --desde video --forzar."))

    # --- volumen ------------------------------------------------------------
    # Los consejos apuntan a volumen_locucion, que es una clave real de
    # config.json desde que se sacó el volume=1.0 escrito a mano del
    # filter_complex. (En el repo hermano estos mismos consejos nombran
    # "volumen_narracion", que allí no existe: la clave es volumen_locucion.)
    lufs = medidas.get("lufs")
    if lufs is not None and lufs == float("-inf"):
        # Silencio digital: hay pista de audio pero no suena nada. Decirlo
        # así, y no como "inf dB por debajo de lo normal", que es lo que
        # sale de restarle -inf a -14.
        hallazgos.append(_hallazgo(
            "mudo", "fallo",
            "La pista de audio existe pero está en silencio absoluto.",
            "El TTS devolvió un archivo vacío o el mapeo de audio del "
            "render apuntó a la pista equivocada. Rehaz el video."))
    ...
    if pico is not None and pico > PICO_MAXIMO:
        hallazgos.append(_hallazgo(
            ...
            "Deja al menos medio decibelio de aire: baja un poco volumen_locucion."))
    ...
    for desde, hasta in medidas.get("negros", []):
        if desde < 0.05:
            hallazgos.append(_hallazgo(
                ...
                "Vuelve a renderizarlo; si se repite, el primer plano empieza en negro."))
        elif hasta - desde >= 0.5:
            hallazgos.append(_hallazgo(
                "negro", "aviso",
                f"{hasta - desde:.1f}s en negro en el segundo {desde:.0f}.",
                "Suele ser un corte entre dos clips de b-roll."))

    perdon = _perdon_congelado(cfg)
    for desde in medidas.get("congelados", []):
        if desde >= perdon:
            hallazgos.append(_hallazgo(
                "congelado", "aviso",
                f"La imagen se queda parada a partir del segundo {desde:.0f}.",
                "El clip de apoyo se acabó antes que la narración y se quedó "
                "en el último cuadro."))

    # --- formato ------------------------------------------------------------
    # Se decide con la duración MEDIDA, no con lo que diga config.json: para
    # eso existe formato_video.es_short_medido. Un video que apuntaba a 60s y
    # salió de 200 es un video largo y tiene que estar en horizontal.
    ancho, alto = medidas.get("ancho"), medidas.get("alto")
    if ancho and alto and dur > 0:
        quiere = (1080, 1920) if formato_video.es_short_medido(dur, cfg) else (1920, 1080)
        if (ancho, alto) != quiere:
            hallazgos.append(_hallazgo(
                "formato", "fallo",
                f"Mide {ancho}x{alto} y para {dur:.0f}s debería medir "
                f"{quiere[0]}x{quiere[1]}.",
...
def guardadas():
    return _leer(RUTA_CALIDAD, {})


def revisar_archivo(ruta, registro=None, cfg=None):
    """Mide y juzga un archivo. Devuelve la entrada lista para guardar."""
    if cfg is None:
        cfg = _config()
    medidas = medir(ruta)
    hallazgos = revisar(medidas, registro, cfg)
    return {
        "cuando": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "dia": (registro or {}).get("dia"),
...
def videos_renderizados():
    """Los del registro del lote cuyo archivo sigue en disco."""
    lote = _leer(RUTA_RESULTADO, {})
    return [v for v in lote.get("completados", []) if archivos.valido(v.get("ruta", ""))]


def revisar_pendientes(todos=False, solo=None, al_terminar=None, cfg=None):
    ...
    medir es lento y la corrida se puede cortar a la mitad.
    ...
    if solo is not None:
        videos = [v for v in videos if v.get("dia") == solo]
...
def main(argv=None):
    ...
    parser.add_argument("--solo", type=int, metavar="N", help="Solo ese día.")
