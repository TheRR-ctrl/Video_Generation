...
No es parte de la corrida diaria del pipeline (la música no necesita cambiar
por video): se pensó para correrse cada tanto (semanal/mensual, manual o con
su propio cron) y sumar variedad a lo que ya hay.
...
PISTAS_POR_TONO = 3  # cuántas pistas mantener por categoría

# Tags de Jamendo por tono (ver detectar_tono_video en generar_video_maestro.py
# para las mismas categorías). Jamendo combina varios tags con lógica "Y"
# (casi nunca hay resultados), así que se consulta un tag a la vez y se van
# juntando resultados de varios.
TAGS_POR_TONO = {
    "reflexivo": ["calm", "piano", "ambient", "thoughtful", "meditation"],
    "inspirador": ["inspiring", "uplifting", "motivational", "epic", "cinematic"],
    "tenso": ["dark", "ambient", "tension", "mysterious", "cinematic"],
    "esperanzador": ["hopeful", "warm", "emotional", "gentle", "piano"],
}

# Licencias Creative Commons que sí permiten uso comercial y derivados
# (necesario para poder mezclar la pista con la narración).
    ...
    for tag in tags:
        for licencia in LICENCIAS_PERMITIDAS:
            ...
            params = {
                ...
                "tags": tag, "license_cc": licencia, "audioformat": "mp32",
...
def main():
    ...
    for tono, tags in TAGS_POR_TONO.items():
        existentes = [f for f in os.listdir(BASE_DIR) if f.startswith(f"musica_{tono}_") and f.endswith(".mp3")]
        faltan = max(0, PISTAS_POR_TONO - len(existentes))
        if faltan == 0:
            logger.info(f"{tono}: ya hay {len(existentes)} pista(s), no se descarga nada.")
            continue

        logger.info(f"{tono}: buscando {faltan} pista(s) nueva(s) (tags: {tags})...")
        try:
            pistas = buscar_pistas(client_id, tags, faltan, ya_descargadas)
        except Exception as exc:
            logger.warning(f"{tono}: fallo al consultar Jamendo: {exc}")
            continue

        for track in pistas:
            nombre_archivo = f"musica_{tono}_{limpiar_nombre_archivo(track.get('artist_name', 'na'))}_{track['id']}.mp3"
